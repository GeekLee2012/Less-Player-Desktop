...
export default { name: 'JellyfinSessionEditView' }
</script>
...
const { addJellyfinSession, updateJellyfinSession, getJellyfinSession,  } = useCloudStorageStore()
...
const loadSession = () => {
    ...
    const session = getJellyfinSession(id)
    if (!session) return
...
const submit = () => {
    ...
    let success = false
    if (isCreateMode) {
        const _title = isBlank(title) ? 'Jellyfin' : title
        const _url = isBlank(url) ? Jellyfin.DEFAULT_SESSION.url : url
        success = addJellyfinSession(_title, _url, username, password, apiKey)
    } else {
        success = updateJellyfinSession(props.id, title, url)
    }
...
<template>
    <div id="jellyfin-session-edit-view">
        <div class="header">
            <span class="title" v-show="!id">新建Jellyfin会话</span>
            <span class="title" v-show="id">编辑Jellyfin会话</span>
        </div>
        <div class="tip-text">
...
#jellyfin-session-edit-view .header {
    display: flex;
    flex-direction: column;
...
#jellyfin-session-edit-view .header .title {
    text-align: left;
    margin-top: 5px;
    /*font-size: 30px;*/
...
#jellyfin-session-edit-view .center {
    display: flex;
    flex-direction: column;
    flex: 1;
    margin-top: 10px;
...
#jellyfin-session-edit-view .tip-text {
    text-align: left;
    margin-bottom: 5px;
}

#jellyfin-session-edit-view .center .form-row {
    margin-bottom: 17px;
    display: flex;
    flex-direction: row;
}

#jellyfin-session-edit-view .center .form-row > div {
    display: flex;
    flex-direction: row;
    align-items: center;
    flex: 9;
...
#jellyfin-session-edit-view .center .form-row > div.sec-title {
    flex: 1;
    min-width: 100px;
}


#jellyfin-session-edit-view .center .form-row input,
#jellyfin-session-edit-view .center .form-row textarea {
    flex: 1;
...
#jellyfin-session-edit-view .center .action {
    display: flex;
    flex-direction: row;
}

#jellyfin-session-edit-view .spacing {
    margin-left: 20px;
}
</style>