...
export default { name: 'DataRestoreView' }
</script>
...
const customPlaylistsLength = ref(0)

const sourcesCategories = reactive([])
const sourcesData = reactive({})
const backupFile = ref("")

/*
const sourcesReference = reactive([ {
    id: "favorites",
    name: "我的收藏",
    children: [{
        id: "songs",
...
const resovleFilename = (filePath) => {
    const path = filePath.replace(/\\/g, '/')
    const from = path.lastIndexOf("/") + 1
    const to = path.lastIndexOf(".json")
...
const openBackupFile = async () => {
    ...
    resetSourcesData()
    backupFile.value = resovleFilename(result.filePath)
...
const getChildItemSuffix = (id, subId) => {
    const state = sourcesData[id][subId]
    ...
    <div id="data-restore-view">
        <div class="header">
            ...
            <div class="action">
                ...
                <SvgTextButton class="spacing" :disabled="!isReady" text="开始还原" :leftAction="restore"></SvgTextButton>
