...
const { recentVideos } = storeToRefs(useVideoPlayStore())
const { getRecentLatestVideo, clearRecentVideos, } = useVideoPlayStore()
...
const playRecentLatest = () => {
    ...
    if(!video) return 
    const { data, index } = video
    showToast('即将为您恢复最近播放', () => playVideo(data, index))
}
