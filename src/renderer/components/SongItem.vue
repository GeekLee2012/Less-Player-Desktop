...
onMounted(() => {
    if(songItemRef.value) {
        if(props.checkbox) {
            songItemRef.value.removeEventListener("contextmenu")
            return 
        }
...
<template>
    <div class="song-item" ref="songItemRef" @click="toggleCheck">
        ...
        <div class="title" :class="{ spacing: !checkbox }">
...
.song-item .title:hover .limitedSpan {
    width: 158px;
    width: 50%;
    overflow: hidden;
    text-overflow: ellipsis;
