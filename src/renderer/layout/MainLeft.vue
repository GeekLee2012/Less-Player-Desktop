...
const { isPlaylistMode, isArtistMode, exploreModeIndex, exploreModeCode } = storeToRefs(useMainViewStore())
...
const updateNavIndex = (index) => {
    ...
    let url = isLocal.value ? '/' + code : ('/' + exploreMode + '/square/' + code)
    router.push(url)
}

const switchExploreMode = () => {
    const index = (exploreModeIndex.value + 1) % 2
